...
function createDialogWindow() {
		...
		targetProjectFolder, frameRate)
	{
		try {
			var mouthCues;
				try{
					basePath = Folder.userData.fullName;
					mouthCues= generateMouthCues(audioFileFootage, recognizer, dialogText, mouthComp,
					extendedMouthShapeNames, targetProjectFolder, frameRate);
				} catch (e) {
							Window.alert("E002 "+e.message, appName, true);
							return;
						}

			try{app.beginUndoGroup(appName + ': Animation');} catch (e) {
						Window.alert("E003 "+e.message, appName, true);
						return;
					}
			try{animateMouthCues(mouthCues, audioFileFootage, mouthComp, targetProjectFolder,
				frameRate);}catch (e) {
							Window.alert("E004 "+e.message, appName, true);
							return;
						}

			try{app.endUndoGroup();}catch (e) {
						Window.alert("E005 "+e.message, appName, true);
						return;
					}

		} catch (e) {
			Window.alert("E001 "+e.message, appName, true);
			return;
		}
	}
